...
for l in ${latent[@]}; do
    for p in ${dropout[@]}; do
        # Train
        CUDA_VISIBLE_DEVICES="${device}" python  train.py -data data/${dataset}/${dataset}-woprod-${beam} \
        ...
        CUDA_VISIBLE_DEVICES=${device} python translate.py -model ${best_model_path} -src data/${dataset}/src-test-woprod-${beam}.txt -output_dir experiments/${dataset}_${model_name}_${l}_${p}_${seed}/preds/ -beam_size=5 -gpu 0 -batch_size=20 -n_best=5 -log_probs -n_translate_latent ${l}
        ...
        if [ ${l} -eq 0 ]; then
            python parse/parse_output.py -input_file experiments/${dataset}_${model_name}_${l}_${p}_${seed}/preds/output -target_file data/${dataset}/tgt1-test.txt -beam_size 50 -result_path experiments/results/${dataset}_${model_name}_${l}_${p}/ -seed ${seed}
