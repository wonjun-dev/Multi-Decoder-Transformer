...
def build_save_dataset(corpus_type, fields, src_reader, opt):
    ...
    for src, tgt, maybe_id in zip(srcs, tgts, ids):
        ...
        src_shards = split_corpus(src, 0) #opt.shard_size)
        tgt_shards = [np.load(tgt)] #split_corpus(tgt, opt.shard_size)
...
def main(opt):
    ...
    fields = dict({})
    src_field_kwargs = {"n_feats": src_nfeats, #n_src_feats,
                        "include_lengths": True,
                        "pad": None, "bos": None, "eos": None,
                        "truncate": opt.src_seq_length_trunc, #src_truncate,
                        "base_name": "src"}
    fields["src"] = text_fields(**src_field_kwargs)
    fields['tgt'] = Field(use_vocab=False, dtype=torch.float, sequential=False)
            #vec_fields(pad_index=-1, include_lengths=False)
    ...
    #tgt_reader = inputters.str2reader["vec"].from_opt(opt)
    tgt_reader = None

    logger.info("Building & saving training data...")
    fields = build_save_dataset(
        'train', fields, src_reader, opt)
