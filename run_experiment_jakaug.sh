...
#USPTO-50k_no_rxn_tgt_aug_2fields
latent=(0) # 2 5)
dropout=(0.25)
model_name=mt
device=$1
...
for l in ${latent[@]}; do
    for p in ${dropout[@]}; do
        # Train
        #CUDA_VISIBLE_DEVICES="${device}" python  train.py -data data/${dataset}/${dataset}-jakaug \
        #        -save_model experiments/${dataset}_jakaug_${model_name}_${l}_${p}_${seed} \
        ...
        #        -heads 8 -transformer_ff 2048 -tensorboard -tensorboard_log_dir runs/${dataset}_jakaug_${model_name} -world_size=1 -seed "${seed}" -early_stopping 15 -n_latent "${l}"

        # Translate
        summ_path=experiments/${dataset}_jakaug_${model_name}_${l}_${p}_${seed}/summary.txt
        line_cnt=0
        sep_cnt=0
        while read line; do
        ...
        #CUDA_VISIBLE_DEVICES=${device} python translate.py -model ${best_model_path} -src data/${dataset}/src-test.txt -output_dir experiments/${dataset}_jakaug_${model_name}_${l}_${p}_${seed}/preds/ -beam_size=50 -gpu 0 -batch_size=20 -n_best=50 -log_probs -n_translate_latent ${l}
        ...
        if [ ${l} -eq 0 ]; then
            python parse/parse_output.py -input_file experiments/${dataset}_jakaug_${model_name}_${l}_${p}_${seed}/preds/output -target_file data/${dataset}/tgt-test.txt -beam_size 50 -result_path experiments/results/${dataset}_jakaug_${model_name}_${l}_${p}/ -seed ${seed}
