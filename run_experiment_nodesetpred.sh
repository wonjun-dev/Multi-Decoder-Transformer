...
for l in ${latent[@]}; do
    for p in ${dropout[@]}; do
        # Train
        CUDA_VISIBLE_DEVICES="${device}" python  train.py -data data/${dataset}/${dataset}-hint-${beam} \
        ...
        CUDA_VISIBLE_DEVICES=${device} python translate.py -model ${best_model_path} -src data/${dataset}/src-test-hint-${beam}.txt -output_dir experiments/${dataset}_${model_name}_${l}_${p}_${seed}/preds/ -beam_size=50 -gpu 0 -batch_size=20 -n_best=50 -log_probs -n_translate_latent ${l}
